//! Safestart videos and functions.
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢹⣿⣵⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣯⡁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
*/
pub const SAFESTART_VIDEO_HEIGHT: i32 = 240;
pub const SAFESTART_VIDEO_WIDTH: i32 = 426;
pub const SAFESTART_VIDEO: [u8; 6272] = [
...
/// Prepends the already encoded SAFESTART_VIDEO to the annex_b_video
pub fn prepend_safe_video(annex_b_video: &[u8]) -> Vec<u8> {
    let mut new_vid = SAFESTART_VIDEO.to_vec();
    new_vid.extend(annex_b_video);

    new_vid
}

/// Returns the SAFESTART_RTP video
pub fn get_rtp_safe_video() -> Vec<Vec<u8>> {
    let mut new_vid = Vec::new();

    new_vid.push(SAFESTART_RTP_0.to_vec());
    new_vid.push(SAFESTART_RTP_1.to_vec());
    new_vid.push(SAFESTART_RTP_2.to_vec());
    new_vid.push(SAFESTART_RTP_3.to_vec());
    new_vid.push(SAFESTART_RTP_4.to_vec());
    new_vid.push(SAFESTART_RTP_5.to_vec());
    new_vid.push(SAFESTART_RTP_6.to_vec());
    new_vid.push(SAFESTART_RTP_7.to_vec());
    new_vid.push(SAFESTART_RTP_8.to_vec());
    new_vid.push(SAFESTART_RTP_9.to_vec());
    new_vid.push(SAFESTART_RTP_10.to_vec());
