...
/// Generate a random Picture Parameter Set to go with a Subset SPS
pub fn random_subset_pps(
    ...
    ds.subset_ppses[subset_pps_idx].available = true;

    ds.subset_ppses[subset_pps_idx].pic_parameter_set_id =
        rconfig.pic_parameter_set_id.sample(film);
    ds.subset_ppses[subset_pps_idx].seq_parameter_set_id = sps.seq_parameter_set_id;

    ds.subset_ppses[subset_pps_idx].entropy_coding_mode_flag =
        rconfig.entropy_coding_mode_flag.sample(film);
    ds.subset_ppses[subset_pps_idx].bottom_field_pic_order_in_frame_present_flag = rconfig
        .bottom_field_pic_order_in_frame_present_flag
        .sample(film);

    ds.subset_ppses[subset_pps_idx].num_slice_groups_minus1 =
        rconfig.num_slice_groups_minus1.sample(film);

    if ds.subset_ppses[subset_pps_idx].num_slice_groups_minus1 > 0 {
        ds.subset_ppses[subset_pps_idx].slice_group_map_type =
            rconfig.slice_group_map_type.sample(film);

        if ds.subset_ppses[subset_pps_idx].slice_group_map_type == 0 {
            for _ in 0..=ds.subset_ppses[subset_pps_idx].num_slice_groups_minus1 {
                ds.subset_ppses[subset_pps_idx]
                    .run_length_minus1
                    .push(rconfig.run_length_minus1.sample(film));
            }
        } else if ds.subset_ppses[subset_pps_idx].slice_group_map_type == 2 {
            for _ in 0..ds.subset_ppses[subset_pps_idx].num_slice_groups_minus1 {
                ds.subset_ppses[subset_pps_idx]
                    .top_left
                    .push(rconfig.top_left.sample(film));
        ...
        } else if ds.subset_ppses[subset_pps_idx].slice_group_map_type == 3
            || ds.subset_ppses[subset_pps_idx].slice_group_map_type == 4
            || ds.subset_ppses[subset_pps_idx].slice_group_map_type == 5
        {
            ds.subset_ppses[subset_pps_idx].slice_group_change_direction_flag =
                rconfig.slice_group_change_direction_flag.sample(film);
            ds.subset_ppses[subset_pps_idx].slice_group_change_rate_minus1 =
                rconfig.slice_group_change_rate_minus1.sample(film);
        } else if ds.subset_ppses[subset_pps_idx].slice_group_map_type == 6 {
            ds.subset_ppses[subset_pps_idx].pic_size_in_map_units_minus1 =
                rconfig.pic_size_in_map_units_minus1.sample(film);

            for _ in 0..=ds.subset_ppses[subset_pps_idx].pic_size_in_map_units_minus1 {
                ds.subset_ppses[subset_pps_idx]
                    .slice_group_id
                    .push(rconfig.slice_group_id.sample(film));
            }
    ...
    ds.subset_ppses[subset_pps_idx].num_ref_idx_l0_default_active_minus1 =
        rconfig.num_ref_idx_l0_default_active_minus1.sample(film);
    ds.subset_ppses[subset_pps_idx].num_ref_idx_l1_default_active_minus1 =
        rconfig.num_ref_idx_l1_default_active_minus1.sample(film);
    ds.subset_ppses[subset_pps_idx].weighted_pred_flag = rconfig.weighted_pred_flag.sample(film);
    ds.subset_ppses[subset_pps_idx].weighted_bipred_idc =
        rconfig.weighted_bipred_idc.sample(film) as u8;
    ds.subset_ppses[subset_pps_idx].pic_init_qp_minus26 = rconfig.pic_init_qp_minus26.sample(film);
    ds.subset_ppses[subset_pps_idx].pic_init_qs_minus26 = rconfig.pic_init_qs_minus26.sample(film);
    ds.subset_ppses[subset_pps_idx].chroma_qp_index_offset =
        rconfig.chroma_qp_index_offset.sample(film);
    ds.subset_ppses[subset_pps_idx].deblocking_filter_control_present_flag =
        rconfig.deblocking_filter_control_present_flag.sample(film);
    ds.subset_ppses[subset_pps_idx].constrained_intra_pred_flag =
        rconfig.constrained_intra_pred_flag.sample(film);
    ds.subset_ppses[subset_pps_idx].redundant_pic_cnt_present_flag =
        rconfig.redundant_pic_cnt_present_flag.sample(film);

    ds.subset_ppses[subset_pps_idx].more_data_flag = rconfig.include_more_data.sample(film);

    // rbsp_more_data()
    if ds.subset_ppses[subset_pps_idx].more_data_flag {
        ds.subset_ppses[subset_pps_idx].transform_8x8_mode_flag =
            rconfig.transform_8x8_mode_flag.sample(film);
        ds.subset_ppses[subset_pps_idx].pic_scaling_matrix_present_flag =
            rconfig.pic_scaling_matrix_present_flag.sample(film);

        if ds.subset_ppses[subset_pps_idx].pic_scaling_matrix_present_flag {
            let max_val = 6 + match sps.chroma_format_idc != 3 {
                true => 2,
                false => 6,
            } * match ds.subset_ppses[subset_pps_idx].transform_8x8_mode_flag {
                true => 1,
                false => 0,
            };
            for i in 0..max_val {
                ...
                if ds.subset_ppses[subset_pps_idx].pic_scaling_list_present_flag[i] {
                    if i < 6 {
                        randomize_pps_scaling_list(
                            &mut ds.subset_ppses[subset_pps_idx].delta_scale_4x4[i],
                            16,
                            rconfig,
                            film,
                        )
                        ...
                        randomize_pps_scaling_list(
                            &mut ds.subset_ppses[subset_pps_idx].delta_scale_8x8[i],
                            64,
                            rconfig,
                            film,
                        )
        ...
        ds.subset_ppses[subset_pps_idx].second_chroma_qp_index_offset =
            rconfig.second_chroma_qp_index_offset.sample(film);
    }
}
