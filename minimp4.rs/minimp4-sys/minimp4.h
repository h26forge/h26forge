...
static inline const unsigned char *next_dsi(const unsigned char *p, const unsigned char *end, int *bytes)
{
    if (p < end + 2)
    {
        *bytes = p[0]*256 + p[1];
...
static int add_sample_descriptor(MP4E_mux_t *mux, track_t *tr, int data_bytes, int duration, int kind)
{
    ...
    smp.duration = (duration ? (unsigned) duration : tr->info.default_duration);
...
)
{
    ...
    unsigned flags;
    // Below is not used so we comment out for now
    //enum
    //{
...
int MP4E_put_sample(MP4E_mux_t *mux, int track_num, const void *data, int data_bytes, int duration, int kind)
{
    ...
    //printf("MP4E_put_sample - track_num: %d ; data_bytes: %d ; duration: %d ; kind: %d \n", track_num, data_bytes, duration, kind);
    if (mux->enable_fragmentation)
    {
        //printf("MP4E_put_sample - fragmentation enabled \n");
    ...
    } else
    {
        if (!mux->sequential_mode_flag)
        {
            ...
            if ((size_t)tr->smpl.bytes < sizeof(sample_t)) {
                //printf("MP4E_put_sample - tr->smpl.bytes: %d :( \n", tr->smpl.bytes);
...
static void patch_slice_header(h264_sps_id_patcher_t *h, bit_reader_t *bs, bs_t *bd)
{
    ...
    //printf(" ------------------------- \n");
    //printf("patch_slice_header - old pps_id %d \n", pps_id);

    pps_id = h->map_pps[pps_id];

    assert(pps_id <= 255);
    //printf("patch_slice_header - new pps_id %d \n", pps_id);
    //printf("patch_slice_header - first_mb_in_slice %d \n", first_mb_in_slice);
    //printf("patch_slice_header - slice_type %d \n", slice_type);
...
static int transcode_nalu(const unsigned char *src, int nalu_bytes, unsigned char *dst)
{
    //int old_id;
    ...
    *dst = *src;
    ...
    switch(payload_type)
    {
    // WRV: For now - ignore changing IDs
    //case 7:
    //    {
    ...
    //            return 0;
    //        h->map_sps[old_id] = id;
    //        printf("transcode_nalu - old sps id %d and new sps id %d \n", old_id, id);
    ...
    //            return 0;
    //        h->map_pps[old_id] = id;
    //        printf("transcode_nalu - old pps id %d and new sps id %d \n", old_id, id);
...
static int mp4_h265_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int sizeof_nal, unsigned timeStamp90kHz_next)
{
    ...
    case HEVC_NAL_SPS:
        ...
        // the 'mdat' atom instead of the hvcC atom. This was to avoid any checks
        // that would occur on the hvcC atom, even with an SPS inside of a stream.
...
int mp4_h26x_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int length, unsigned timeStamp90kHz_next)
{
    ...
    for (;;nal++)
    {
        ...
        // WRV Note: We want AUDs to show up in the stream
        //if (9 == payload_type)
...
exit_with_free:
        ...
        sizeof_nal = transcode_nalu(nal2, sizeof_nal, nal1);
        ...
        case 7:
            if (h->need_sps) {
                ...
                // If we don't need an SPS, then just throw it into the stream instead of AVCC
                int sample_kind = MP4E_SAMPLE_DEFAULT;
        ...
        case 8:
            ...
            h->need_pps = 0;
            break;
        case 6: // Just copy in SEI values
            {
                int sample_kind = MP4E_SAMPLE_DEFAULT;
                nal2[0] = (unsigned char)((sizeof_nal - 4) >> 24);
                nal2[1] = (unsigned char)((sizeof_nal - 4) >> 16);
        ...
        default:
            if (h->need_sps)
                goto exit_with_free;
            if (!h->need_pps ) // && !h->need_idr) WRV NOTE: sometimes we don't care about I slices?
            {
                // WRV NOTE: because we are generating slices with first_mb_in_slice greater than 0 that
                //       do not continue from previous ones, we comment out this path and treat them
                //       as regular slices
                //bit_reader_t bs[1];
                ...
                if (payload_type == 5)
                    sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
                err = MP4E_put_sample(h->mux, h->mux_track_id, nal2, sizeof_nal, timeStamp90kHz_next, sample_kind);
                //printf("err from MP4E_put_sample: %d \n", err);
            }
            break;
        }
        free(nal1);
...
//         case BOX_svc1:   - no test
        ...
        {
            if (payload_bytes > (uint64_t)file_size)
            {
                eof_flag = 1;
            } else
            {
